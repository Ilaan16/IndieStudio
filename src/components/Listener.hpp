...
namespace indie {
    class Listener : public IComponent {
        public:
            ...
            bool addEvent(const KeyboardKey &key, std::shared_ptr<Event> &handler);
            bool addEvent(const MouseButton &mouse, std::shared_ptr<MouseEvent> &handler);
        ...
        private:
            std::unordered_map<KeyboardKey, std::shared_ptr<indie::Event>> keyboardEvents;
            std::unordered_map<MouseButton, std::shared_ptr<indie::MouseEvent>> mouseEvents;
