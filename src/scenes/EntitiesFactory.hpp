...
namespace indie {
    class EntitiesFactory {
        public:
            ...
            void createPlayer(std::map<typeEntity ,std::vector<std::shared_ptr<Entity>>> &_entities, std::string model, std::string texture, Vector3D pos, float angle, Vector3D size, std::string animation);
